# -*- coding: utf-8 -*-
"""
Created on Thu Jul  1 21:26:32 2021
...
def DaeData_split(cla_Data, cla_Labels, cla_flag):
    ...
    cla_flag=[0]*class_num#高光谱地物分类的类数
    
    
    hsi_flag=[0]*class_num #记录每一类的标签 
    
    class_num=0
    
    for i in range(len(num)):
        if num[i]!=0:
            cla_flag[class_num]=num[i]
            
            hsi_flag[class_num]=i
            
            class_num+=1
    
    windowSize=1
    
    split_AugData = np.zeros((aug_data*class_num,1,1, band_shuliang))
    split_Auglabels = np.zeros((aug_data*class_num))
    ...
    for i in range(len(cla_flag)):
        if cla_flag[i]>=aug_data:
            samp_Data = cla_Data[cla_Labels==hsi_flag[i],:]
            samp_Labels = cla_Labels[cla_Labels==hsi_flag[i]]
            a=[]
            for xx in range(cla_flag[i]):
                a.append(xx)
            random.shuffle(a)
...
class MTLnet(nn.Module):
    def __init__(self):
        ...
        self.towerb1=nn.Sequential(
                nn.Conv3d(16,1,kernel_size=[1,1,3],stride=1,padding=(0,0,1)),#4*3*3
                nn.BatchNorm3d(1),
                nn.ReLU(inplace=True),
                )
        
        self.towerb2 = nn.Sequential(
                nn.ConvTranspose3d(1,8,kernel_size=[1,1,6],stride=4,padding=(0,0,3)),#1*9*9
                nn.BatchNorm3d(8),
                nn.ReLU(inplace=True),
    ...
    def forward(self,x1,x2):
        ...
        out1 = self.conva1(x1)
        out1 = self.sharedlayer(out1)
        out1 = out1.view(out1.size(0), -1)
        s1 = self.fc2(out1)        
        
        out2 = self.convb1(x2)
        out2 = self.sharedlayer(out2)
        out2 = self.towerb1(out2)        
        out2 = self.towerb2(out2)
        #out1 = out1.view(out1.size(0), -1)
...
data_path_indian1 = root+'//denoiseHSI//SalinasA.mat'
label_path_indian1 = root+'//denoiseHSI//SalinasA_gt.mat'

#label_path_indian = root+'//denoiseHSI//Indian_pines_gt.mat'
#data_path_indian = root+'//denoiseHSI//WHU_Hi_HongHu.mat'
#label_path_indian = root+'//denoiseHSI//WHU_Hi_HongHu_gt.mat'

data_path1 = data_path_indian1
label_path1 = label_path_indian1

X1 = sio.loadmat(data_path1)
...
y_label1=sio.loadmat(label_path_indian1)

#y_label = y_label['indian_pines_gt']

y_label1 = y_label1['salinasA_gt']
...
cla_Data1,cla_Labels1,cla_flag1 = createImageCubes(X1, y_label1,windowSize=patch_size)

#aaa_data,aaa_label=data_augmentation(cla_Data,cla_Labels,cla_flag)

auged_data1,auged_labels1=DaeData_split(cla_Data1,cla_Labels1,cla_flag1)

Xtrain1 = auged_data1

ytrain1  = auged_labels1

#Xtrain = Xtrain.transpose(0, 3, 1, 2)

#Xtest  = Xtest.transpose(0, 3, 1, 2)

print('after transpose: Xtrain shape: ', Xtrain1.shape)

#print('after transpose: Xtest  shape: ', Xtest.shape)

trainset1 = TrainDS(Xtrain1, ytrain1)

train_loader1 = torch.utils.data.DataLoader(dataset=trainset1, batch_size=128, shuffle=True)
...
for it in range(epoch):
    ...
    for  indx,data in enumerate(zip(train_loader,train_loader1)):
        
        #imagex = train_loader1[indx].to(device) #自编码器
        #labelx = train_loader1[indx].to(device) #自编码器标签
        
        image = data[0][0].to(device)
        label = data[0][1].to(device)

        imagex = data[1][0].to(device)
        labelx = data[1][1].to(device)  
        
        optimer.zero_grad()
                    
        out,out1 = model(image,imagex)
        
        #label = label.unsqueeze(1)
        
        loss1 = criterion(out, label)
        loss2 = criterion(out1, labelx)
...
test_data = TestDS(cla_Data1,cla_Labels1)
